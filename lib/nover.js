...
var prepareFilename = function (filename) {
    'use strict';
    filename = filename.replace(/^\$ROOT/, sys.root);
    filename = filename.replace(/^\$NOVER/, __dirname);
    filename = filename.replace(/^\$PWD/, process.cwd());
    return filename;
};
...
var interpretSockets = function (sockets, app) {
    ...
    if (sockets) {
        sockets.forEach(function (socket) {
            ...
            var filename;
            if (local) {
                var httpd = httpServer.createServer(app);
                filename = prepareFilename(local);
                require(filename).__socket(httpd);
                console.log('Loaded WebSocket "'.green + local +
                            '" at port "'.green + port + '".'.green);
                httpd.listen(port);
            }
        });
    }
